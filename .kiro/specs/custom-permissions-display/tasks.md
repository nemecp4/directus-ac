...
- [x] 1. Update name generation and model validation
  - [x] 1.1 Update `generate_permission_name` signature and implementation in `directus_ac/generate.py`
...
- [x] 3. Simplify check command display
  ...
  - [x] 3.2 Write property test for format custom ref (Property 5)
    - **Property 5: Format custom ref returns only the name**
    - **Validates: Requirements 4.1, 4.2, 4.3, 4.4**

- [x] 4. Update config parser pattern and validation
  - [x] 4.1 Update `_CUSTOM_REF_PATTERN` in `directus_ac/config.py`
    - Change from `re.compile(r"^C_\d+$")` to `re.compile(r"^(CREATE|READ|UPDATE|DELETE)_C_\d+$")`
    - _Requirements: 6.1, 6.2_

  - [x] 4.2 Update `_parse_custom_permissions` error message in `directus_ac/config.py`
  ...
  - [x] 4.3 Write property test for Permission_Set parser (Property 7)
...
- [x] 6. Integration verification
  - [x] 6.1 Wire together and verify round-trip consistency
  ...
  - [x] 6.2 Write property test for config round-trip (Property 4)
