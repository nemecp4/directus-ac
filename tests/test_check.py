...
class TestInlineCustomPermissions:
    ...
    def test_custom_permission_with_validation_inline(self) -> None:
        ...
        assert "articles: read, UPDATE_C_1" in result
    ...
    def test_custom_permission_with_all_indicators_inline(self) -> None:
        ...
        result = cmd._format_policies(permissions, policies, roles)
        assert "UPDATE_C_1" in result
        # Verify no indicators are present
    ...
    def test_multiple_custom_permissions_inline(self) -> None:
        ...
        result = cmd._format_policies(permissions, policies, roles)
        # Standard action first, then custom refs (only names, no indicators)
        assert "articles: read, UPDATE_C_1, CREATE_C_2" in result
    ...
    def test_run_shows_custom_permissions_inline(self, caplog) -> None:
        ...
        assert "UPDATE_C_1" in output
        # No indicators should be present
        assert "has validation" not in output
    ...
    def test_mixed_standard_and_custom_same_collection(self) -> None:
        ...
        result = cmd._format_policies(permissions, policies, roles)
        # Standard actions first, then custom refs (only names, no indicators)
        assert "articles: read, create, UPDATE_C_1, DELETE_C_2" in result
